...
public class Phoenix_Held_Transaction_Approve extends BaseTest {
	...
	public void TransactionStatusafterMangers_approval (ListOrderedMap<String,String> values)
	{	
		try {
			PhoenixLogin phoenixlogin = new PhoenixLogin(driver);
			phoenixlogin.setusername(values.get("User Name"));
			...
			claimtransaction2.ClickOnTransDrop();
			takeScreenShot(driver, "Transaction status after Manager's approval");
			String ActualClaimStatus= claimtransaction2.GetClaimStatus();
			softassert = Assertion.assertEquals(ActualClaimStatus, "", softassert);
			softassert = assertAll(softassert, "Transaction Status after Manger's approval: "+ ActualClaimStatus);
